        ...
        handlers._tokens.verifyToken(token, phone, function (tokenIsValid) {
            if (tokenIsValid) {
                ...
                _data.read('users', phone, function (err, userData) {
                    if (!err && userData) {
                        // Remove the hashed password form userObject before returning it to the request
...
// Users - put
// Required Field: phone
//Optional Field: firstName, lastName, password
                        ...
                        if (!err && userData) {
                            ...
                            _data.update('users', phone, userData, function (err) {
                                if (!err) {
                                    ...
                                    callback(500, {'Error': 'Cannot updated the specified user'});
                                }
                            })
                        } else {
                                       ...
                                       _data.delete('checks', checkId, function(err){
                                           ...
                                           if(checksDeleted == checksToDelete){
                                               if(!deletionErrors){
                                                   ...
                                                   callback(500, {'Error': 'Errors encountered while deleting user checks, all checks might not get deleted from the system.'})
                                               }
                                           }
                                       }) ;
                                    });
        ...
        _data.read('tokens', id, function(err, tokenData){
            if(!err && tokenData) {
                ...
                if(tokenData.expires > Date.now()){
                    ...
                    _data.updates('tokens', id, tokenData, function(err){
                        if(!err) {
                            callback(200);
                        }else {
                ...
                _data.read('users', userPhone, function(err, userData){
                   if(!err && userData) {
                       ...
                       if(userChecks.length < config.maxChecks) {
                           ...
                           callback(400, {'Error': 'The user already has them acimum number of checks('+config.maxChecks+')'});
                       }
                   }  else {
                       callback(403);
                   }
    ...
    if(id) {
        ...
        _data.read('checks', id, function(err, checkData){
           if(!err && checkData){
               ...
               // Verify that the given token is valid for a the phone number
               handlers._tokens.verifyToken(token, checkData.userPhone, function(tokenIsValid){
                  if(tokenIsValid){
                            ...
                            _data.read('users', checkData.userPhone, function(err, userData){
                               if(!err && userData){
                                   ...
                                   if(checkPosition > -1){
                                       ...
                                       callback(500, {'Error': 'Could not find the check on user object, co could not delete it'});
                                   }
                               } else {
